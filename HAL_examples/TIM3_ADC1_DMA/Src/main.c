...
int main(void)
{
    ...
    {
        HAL_ADC_Start(&hadc1); // works, because DMA1_Channel1 is defined inside ADC1_Init()
        //HAL_ADC_Start_DMA(&hadc1, &AD_RES, 1); // this callback does not work
        //HAL_DMA_Start_IT(&hdma_adc1, &hadc1, &AD_RES, 1); // <= not work
        HAL_Delay(100);
    }

    // Method 4: use TIM3 to trigger ADC1

    HAL_ADC_Start(&hadc1); // start only once
    while (1)
    {   // in the loop, ADC1 should be triggered by TIM3 periodically
        HAL_Delay(100);
    }

}

// This callback is for the interrupt generated from HAL_ADC_Start_IT(&hadc1)
/*
...
//
// This callback works, it is for the interrupt generated by DMA1_Channel1 defined inside ADC1_Init()
//
