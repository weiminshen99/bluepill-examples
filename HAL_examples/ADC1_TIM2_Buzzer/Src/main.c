...
int main(void)
{
    ...
    while (1) // Method3: generate a DMA upon completion
    {
        ...
        //HAL_ADC_Start_DMA(&hadc1, &AD_RES, 1); // <== the callback does not work

        HAL_ADC_Start(&hadc1); // <= DMA1_Channel1 is defined inside ADC1_Init()

        //HAL_DMA_Start_IT(&hdma_adc1, &hadc1, &AD_RES, 1); // <= another way I tried
